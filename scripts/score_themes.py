...
def write_scorecard(output,report):
    rubric=json.loads((ROOT/'evaluation/rubric.json').read_text());results=[]
    for theme in report['themes']:
        name=theme['theme']['id'];shots=json.loads((output/(name+'.cells.json')).read_text())
        from evaluation_gates import evaluate_gates
        results.append({'theme':name,**score(shots,rubric),'gates':evaluate_gates(shots,rubric)})
