...
def run(out):
    ...
    adapter=check_adapter(json.loads((ROOT/'evaluation/themes.json').read_text())[0])
    rubric=json.loads((ROOT/'evaluation/rubric.json').read_text());rubric['inline_oracles']={}
    cases=[]
    for name,token in [('boundary','='),('literal','_')]:
        after=ROOT/'evaluation/fixtures/holdout'/f'{name}.after.py'
    ...
    report={'pass':all(r['pass'] for r in results),'results':results,'scope':'Native Neovim held-out fixtures; no palette optimization, no comfort or Ghostty claim. Positive controls must pass; each negative control must trigger its specific gate.','fixture_hashes':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in (ROOT/'evaluation/fixtures/holdout').glob('*.py')}}
