...
def run(entries,out,nvim=None,include_fzf=True):
    ...
    records=[];results=[]
    fixture=ROOT/'evaluation/fixtures/fzf/history.txt'
    with tempfile.TemporaryDirectory() as tmp:
    ...
    report={'adapters':entries,'palette_sha256':hashlib.sha256((ROOT/'palette/ithilien-dawn.json').read_bytes()).hexdigest(),'results':results,'fzf_version':subprocess.check_output([fzf,'--version'],text=True).strip() if fzf else None,'nvim_version':subprocess.check_output([nvim,'--version'],text=True).splitlines()[0], 'fixture_sha256':hashlib.sha256(fixture.read_bytes()).hexdigest(),'render_profile':json.loads((ROOT/'evaluation/render-profile.json').read_text()),'scope':'Native fzf rendered through Neovim terminal emulator; native Neovim diagnostics and completion. Plugin group contracts only, not plugin workflows. No Ghostty pixels, cursor glyph proof, or comfort score.'}
