...
def generate(width,state,filename,variant='dawn'):
    ...
    meta={'fixture_sha256':{name:hashlib.sha256((ROOT/f'tests/fixtures/readme/{name}.py').read_bytes()).hexdigest() for name in ('before','after')},'palette_sha256':hashlib.sha256((ROOT/'scripts/palette/ithilien.json').read_bytes()).hexdigest(),'nvim':subprocess.check_output([nvim,'--version'],text=True).splitlines()[0],'theme_source_sha256':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted((ROOT/'lua/ithilien').rglob('*.lua'))},'kind':f'Native Neovim UI cells rasterized using Berkeley Mono {"Medium" if variant=="dawn" else "Retina"} 16pt; not a terminal screenshot'}
