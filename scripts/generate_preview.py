...
def generate(width,state,filename):
    ...
    meta={'palette_sha256':hashlib.sha256((ROOT/'scripts/palette/ithilien-dawn.json').read_bytes()).hexdigest(),'nvim':subprocess.check_output([nvim,'--version'],text=True).splitlines()[0],'kanso_revision':subprocess.check_output(['git','-C',str(kanso),'rev-parse','HEAD'],text=True).strip(),'kind':'Native Neovim UI cells rasterized using Berkeley Mono Medium 16pt; not a terminal screenshot'}
