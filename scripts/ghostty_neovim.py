...
def fixtures(output):
    records = []
    available = shutil.which('nvim') and (ROOT/'evaluation/deps/kanso/lua/kanso').is_dir()
    for scene in SCENES:
        name = 'neovim-'+scene
        ...
        records.append({'id': name, 'kind': 'native-neovim',
                        ...
                        'reason': '' if available else 'Neovim or pinned Kanso is missing',
                        'request': path.name,
...
def launch(request, ready, release):
    ...
    init.write_text('vim.opt.rtp:prepend('+json.dumps(str(ROOT/'evaluation/deps/kanso'))+')\n'
                    'vim.opt.rtp:prepend('+json.dumps(str(ROOT))+')\n'
...
def assess(image_path, cells_path, palette, reference):
    from ghostty_quality import srgb_image, grid, cell_checks, rgb
    from ghostty_glyphs import crop_cell, mask, classify, templates_from_capture, reference_sheet, REFERENCE_GLYPHS
    ...
    contracts=[]
    if any(c['bold'] or c['underline'] for c in emphasized): contracts.append('Diff emphasis must retain ordinary weight without underline')
    if shot['scene']=='diff':
