...
def prepare(fetch=False):
    ...
    return plugin_dependencies('evaluation/pickers-dependencies.json', fetch)
...
"""}
    ...
    for scene in SCENES:
        for width in widths:
            for state in ('initial','reload'):
                ...
                try:
                    shot=isolated_capture(adapter,width,scene,nvim=shutil.which('nvim'),action=state,workflow=str(ROOT/'evaluation/pickers.lua'))
    ...
    report={'title':'Picker and completion workflows','pass':all(r['pass'] for r in results),'results':results,'dependencies':deps,'nvim':subprocess.check_output(['nvim','--version'],text=True).splitlines()[0],'render_profile':json.loads((ROOT/'evaluation/render-profile.json').read_text()),'palette_sha256':hashlib.sha256(palette).hexdigest(),'fixture_sha256':hashlib.sha256((ROOT/'evaluation/pickers.lua').read_bytes()).hexdigest(),'scope':'Actual Telescope, Snacks picker and nvim-cmp renderers with deterministic Python fixtures. Native Neovim cells, not Ghostty pixels or comfort proof.'}
...
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--output',type=Path,default=ROOT/'evaluation/results/pickers')
