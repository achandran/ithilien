...
def plugin_dependencies(manifest, fetch=False):
    deps = json.loads((ROOT / manifest).read_text())
    if fetch:
        for d in deps.values():
            fetch_one(d)
    validate(deps)
...
def dependencies(themes=None, build_only=False):
    ...
    for entry in entries:
        if themes and entry['id'] not in themes and entry['id'] != 'ithilien-dawn':
            continue
    ...
    if not build_only:
        for name in ('pickers', 'python-tools', 'git-review'):
            for d in json.loads((ROOT / f'evaluation/{name}-dependencies.json').read_text()).values():
                add(d)
        py = json.loads((ROOT / 'evaluation/python-runtime.json').read_text())
        codex = json.loads((ROOT / 'evaluation/sources.json').read_text())['codex']
...
if __name__ == '__main__':
    ...
    p.add_argument('--themes', nargs='+', default=['ithilien-dawn'])
    a = p.parse_args()
