...
mod ithilien_evaluation {
    ...
    fn exported_theme_native_cells() {
        ...
        let mut reader = std::io::BufReader::new(std::fs::File::open(std::env::var("ITHILIEN_CODEX_THEME").map(PathBuf::from).unwrap_or_else(|_|root.join("extras/codex/themes/ithilien-dawn.tmTheme"))).unwrap());
        ...
        for (file, lang) in [("codex/rust.after.rs", "rust"), ("codex/py.after.py", "python"), ("python/python-models.after.py", "python"), ("python/python-async.after.py", "python"), ("python/python-expressions.after.py", "python")] {
            let code = std::fs::read_to_string(root.join("evaluation/fixtures").join(file)).unwrap();
