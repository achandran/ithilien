...
All three use Berkeley Mono Medium, 14 pt, no thickening, full faint opacity, and minimum-contrast 1. This isolates color differences; it is not each theme's untouched default experience. Upstream Kanso and Zenbones have dark block cursors with light text, which intentionally remain visible in the comparison rather than being redesigned to meet Dawn's dark-text rule. Kanso's omitted cursor-text is explicitly set to its light canvas.
