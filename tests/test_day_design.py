...
class DayDesign(unittest.TestCase):
    def audit(self, palette):
        with tempfile.TemporaryDirectory() as temp, patch.object(audit_palette,'ROOT',Path(temp)), patch.object(audit_palette,'load_palette',return_value=palette), contextlib.redirect_stdout(io.StringIO()):
            ...
            return json.loads((Path(temp)/'evaluation/results/palette/ithilien-dawn-audit.json').read_text())

    def test_rosehip_selection_does_not_change_diff_emphasis(self):
        p=load_palette('ithilien-dawn')
        self.assertEqual(p['highlight']['background'], '#B8595C')
    ...
    def test_interaction_minimum_is_not_waived(self):
        p=copy.deepcopy(load_palette('ithilien-dawn'))
        p['highlight']['background']='#8B3037'
        p['highlight']['foreground']='#000000'
        self.assertIn('highlighted text',self.audit(p)['failures'])
    ...
    def test_surface_order(self):
        ...
        self.assertEqual(p['surface2'],p['crust'])
        values=[oklch(p[k])[0] for k in ['base','surface1','mantle','crust']]
